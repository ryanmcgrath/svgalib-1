...
LRMI_init(void)
	...
	*/
	fd_mem = __svgalib_mem_fd;

	if (fd_mem == -1)
		{
		perror("open /dev/svga");
		return 0;
		}
...
em_inb(int port)
	{
   	context.vm.regs.eax&=0xffffff00;
   	context.vm.regs.eax|=port_in(port);
	}

static void
em_inw(int port)
	{
   	context.vm.regs.eax&=0xffff0000;
   	context.vm.regs.eax|=port_inw(port);
	}

static void
em_inl(int port)
	{
   	context.vm.regs.eax=port_inl(port);
	}

static void
em_outb(int port)
	{
   	port_out(context.vm.regs.eax,port);
	}

static void
em_outw(int port)
	{
   	port_outw(context.vm.regs.eax,port);
	}

static void
em_outl(int port)
	{
   	port_outl(context.vm.regs.eax,port);
	}
	...
	while (1)
		...
		if (insn[i] == 0x66)
		 ...
		 || insn[i] == 0x67)
			...
			{
			em_inb(context.vm.regs.edx&0xffff);
			i++;
			break;
			}
		else if (insn[i] == 0xed)
			{
			if (prefix.size)
				em_inl(context.vm.regs.edx&0xffff);
			else
				em_inw(context.vm.regs.edx&0xffff);
			i++;
			break;
			}
		else if (insn[i] == 0xee)
			{
			em_outb(context.vm.regs.edx&0xffff);
			i++;
			break;
			}
		else if (insn[i] == 0xef)
			{
			if (prefix.size)
				em_outl(context.vm.regs.edx&0xffff);
			else
				em_outw(context.vm.regs.edx&0xffff);
...
LRMI_call(struct vm86_regs *r)
	...
	context.vm.regs.eip = r->eip & 0xffff;

	if (r->ss == 0 && (r->esp & 0xffff) == 0)
		{
		...
		context.vm.regs.esp = r->esp & 0xffff;
		}
...
static int
LRMI_int(int i, struct vm86_regs *r)
	{
	unsigned int vret;
...
LRMI_callbacks __svgalib_default_LRMI_callbacks = {
	.rm_init = LRMI_init,
	.rm_call = LRMI_call,
	.rm_int = LRMI_int,
	.rm_alloc_real = LRMI_alloc_real,
	.rm_free_real = LRMI_free_real,
};
